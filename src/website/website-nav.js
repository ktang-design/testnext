...
// Uses the canonical .nav-item / .sidenav classes from
// components/navigation.css (collapsible groups: .nav-item__chevron, .is-open,
// .nav-item--secondary, .sidenav__subitem[hidden]).
(function () {
  ...
  // Search and Navigation are no longer their own top-level pages in the nav —
  // they (and Header's own settings) are grouped as children under Header.
  // "Navigation menu" shows disabled/40% in the Figma mock, but stays a fully
  // normal, selectable item here per explicit instruction: only the visual mock
  // dims it, the real nav does not.
  var NAV = [
  ...
  var GROUPS_KEY = 'website-nav-groups';
  var savedGroups = {};
  ...
  mount.querySelectorAll('[data-nav-group]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      // group icon navigates to its first child (Header -> Navigation menu).
      // When expanded, fall through to the normal expand/collapse toggle below —
      // clicking Header there only opens/closes the group, it does not navigate.
      ...
      savedGroups[btn.getAttribute('data-group-slug')] = open;
